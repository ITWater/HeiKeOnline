...
*/
@Service
public class AdminMsgOfNotLegalCommentWebSocketService {
	@Autowired
	...
	private static final String KindOf="NotLegalComments";
	...
	public void saveAndSendAdminMsgOfNotLegalComment(AdminMsg adminMsg){
		...
		String cacheKey="notRead_adminMsg_notLegalComment";
		//存入缓存 先弹出后选择存入
		...
		for (WebSocketSession adminSession : AdminWebSocketPushHandler.users.values()) {
			if (adminSession.isOpen()) {
				...
				try {
					...
					logger.error("webSocket消息adminMsg_NotLegalComment发送失败"+e.getMessage());
				}
			}
		}
		//如果所有的管理员都不在线
